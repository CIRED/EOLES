...
from utils2 import get_config, read_constant, read_profile

# Functions used to build the model
from utils2 import calculate_annuities_capex, calculate_annuities_storage_capex, update_vom_costs_scc, \
    define_month_hours, shift_range

# Functions used to process outputs
from utils2 import get_technical_cost, extract_curtailment, extract_hourly_balance, \
...
from utils2 import plot_load_shift_week, plot_elec_balance_week, plot_elec_residual_balance_week, \
...
class ModelEOLES():
    ...
    def load_inputs(self):
        ...
        self.vOM = read_constant(self.config["vOM"]) # 1e6€/GW = €/kWh
    ...
    def define_sets(self):
        ...
        self.model.all_tech = Set(initialize=["offshore_float", "offshore_ground", "onshore", "pv_ground_S", "pv_ground_EW", "pv_roof_com_S", "pv_roof_com_EW", "pv_roof_indiv_S", "pv_roof_indiv_EW",
                                                 ...
                                                 "phs", "battery_1h", "battery_4h", "h2_saltcavern", "ch4_reservoir", "coge_reservoir", "injection_reservoir", "str_dummy", "import"])
        ...
        self.model.prod_tech = Set(initialize=["offshore_float", "offshore_ground", "onshore", "pv_ground_S", "pv_ground_EW", "pv_roof_com_S", "pv_roof_com_EW", "pv_roof_indiv_S", "pv_roof_indiv_EW",
                                                  "river", "lake", "nuclear", "pyrogazification", "ocgt_coge",
                                                  "natural_gas", "coal", "biomass_coge", "geothermal_coge", "waste", "marine", "rsv_dummy", "import"])
        ...
        self.model.elec_prod = Set(initialize=["offshore_float", "offshore_ground", "onshore", "pv_ground_S", "pv_ground_EW", "pv_roof_com_S", "pv_roof_com_EW", "pv_roof_indiv_S", "pv_roof_indiv_EW",
                                               "river", "lake", "nuclear", "coal", "biomass_coge", "geothermal_coge", "waste", "marine", "ocgt_coge", "import"])
        self.model.CH4_prod = Set(initialize=["pyrogazification", "methanation", "natural_gas"])
        self.model.H2_prod = Set(initialize=["electrolysis"])
        

        # Technologies using each energy vector
        self.model.use_elec = Set(initialize=["phs", "battery_1h", "battery_4h", "electrolysis", "methanation", "str_dummy", "methanization", "methanization_injection"])
        self.model.use_CH4 = Set(initialize=["ch4_reservoir", "ch4_ocgt", "ch4_ccgt"])
        self.model.use_H2 = Set(initialize=["h2_saltcavern", "h2_ccgt"])
        self.model.use_biogas = Set(initialize=["methanization_coge", "methanization_injection", "coge_reservoir", "injection_reservoir"])
        ...
        self.model.biogas_balance = Set(initialize=["methanization", "coge_reservoir", "injection_reservoir"])

        # Conversion technologies
        self.model.conversion_tech = Set(initialize=["ch4_ocgt", "ch4_ccgt", "h2_ccgt", "electrolysis", "methanation", "methanization", "methanization_coge"])
        self.model.double_conversion_tech = Set(initialize=["methanization_injection"])
        self.model.toute_conversion_tech = Set(initialize=["ch4_ocgt", "ch4_ccgt", "h2_ccgt", "electrolysis", "methanation", "methanization", "methanization_coge", "methanization_injection"])
        ...
        self.model.str = \
            Set(initialize=["phs", "battery_1h", "battery_4h", "h2_saltcavern", "ch4_reservoir", "str_dummy", "coge_reservoir", "injection_reservoir"])
        ...
        # Biogas storage
        self.model.str_biogas = Set(initialize=["coge_reservoir", "injection_reservoir"])
    ...
    def define_constraints(self):
        ...
        def str_biogas_simultaneous_limit_constraint_rule(model, h, str_biogas):
            """Constraint for limiting simultaneous upwards and downwards load shift.
            Without this constraint, batteries are able to charge and discharge at maximum power at the same time"""
            return model.str_input[str_biogas, h] + model.str_output[str_biogas, h] <= model.discharging_power[str_biogas]

        self.model.str_biogas_simultaneous_limit_constraint = Constraint(self.model.h, self.model.str_biogas, rule=str_biogas_simultaneous_limit_constraint_rule)
        ...
        def biogas_balance_constraint_rule(model, h):
            ...
            for tech in model.use_biogas:
                ...
                if tech in model.conversion_tech:
                    usage_h += model.conv_output[tech, h]/self.conversion_efficiency.at[tech]


            return supply_h/100 >= usage_h/100   # GW
        ...
        def biogas_annual_demand_constraint_rule(model, y):
            ...
            for tech in model.use_biogas:
                ...
                if tech in model.conversion_tech:
                   ...
                   # usage_tot += sum(model.frr[tech, h]/self.conversion_efficiency.at[tech]*self.reserve_activation_rate.at["frr"] for h in range(8760*y,8760*(y+1)))
                   # usage_tot += sum(model.fcr[tech, h]/self.conversion_efficiency.at[tech]*self.reserve_activation_rate.at["fcr"] for h in range(8760*y,8760*(y+1)))
            # usage_tot += self.biogas_demand

            return supply_tot/100 >= usage_tot/100

       # if not self.biogas_demand_is_profile:
        ...
        def objective_rule(model):
            ...
            return (
                ...
                (sum((model.nominal_power[tech] - self.existing_capacity.at[tech]) * self.annuities.at[tech] for tech in model.prod_tech)
                    ...
                    + sum(model.gene["import", h] for h in model.h) * self.capex.at["import"]
    ...
    def extract_optimisation_results(self):
        ...
        self.summary, self.generation_per_technology = \
                            extract_summary(self.system_social_cost, self.model, self.elec_demand,
                                            ...
                                            self.hourly_balance, self.spot_price, self.capex)
        ...
        self.lcoe_per_tech = calculate_lcoe_per_tech(self.model, self.hourly_balance, self.annuities, self.storage_annuities, self.fOM, self.vOM,
                                                     ...
                                                     self.existing_capacity, self.existing_energy_capacity, self.capex) # €/MWh
